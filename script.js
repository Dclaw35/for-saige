...
const secretsToggle = document.getElementById('secrets-toggle');
const secretsPanel = document.getElementById('secrets-panel');
const secretsMore = document.getElementById('secrets-more');
const secretsClose = document.getElementById('secrets-close');
...
function showSlide(nextIndex, manual=false){ if(manual) userInteracted = true; clearTimeout(introTimer); index = Math.max(0, Math.min(total - 1, nextIndex)); const slide = slides[index]; slides.forEach((s,i)=>s.classList.toggle('active', i===index)); document.querySelectorAll('.sequence-note.show-note').forEach(note=>{ if(!slide.contains(note)){ note.classList.remove('show-note'); } }); updateCounter(); progressBar.style.width = ((index+1)/total*100)+'%'; scrubber.value = String(index + 1); if(scrubber){ const scrubPct = total > 1 ? (index/(total-1)*100) : 0; scrubber.style.setProperty('--scrub-progress', scrubPct + '%'); } prevBtn.style.opacity = index===0 ? '.35' : '1'; nextBtn.textContent = index===total-1 ? 'Start Over' : 'Next'; if(index > 7) hint.style.opacity = '.28'; document.body.classList.toggle('quiet-mode', slide.classList.contains('quiet')); document.body.classList.toggle('dark-moment', slide.classList.contains('special-darken')); document.body.classList.toggle('tv-mode', slide.classList.contains('scene-tv')); document.body.classList.toggle('hush-mode', slide.classList.contains('scene-you') || slide.classList.contains('scene-cleanbeat')); document.body.classList.toggle('hush-lite', slide.classList.contains('scene-seen')); document.body.classList.toggle('scene-hush', slide.classList.contains('scene-hush')); if(progressEl && (slide.classList.contains('kind-chapter') || slide.classList.contains('kind-title'))){ progressEl.classList.remove('section-glow'); void progressEl.offsetWidth; progressEl.classList.add('section-glow'); setTimeout(()=>progressEl.classList.remove('section-glow'), 1300); } if(!userInteracted && !isPlaying && index < 3){ const introDur = Number(slide.dataset.duration || 2400); introTimer = setTimeout(()=>showSlide(index+1, false), introDur); } if(isPlaying) scheduleAutoplay(); }
...
function setupLittleSecrets(){
  if(!secretsToggle || !secretsPanel) return;
  const openPanel = () => {
    secretsPanel.classList.add('is-open');
    secretsToggle.classList.add('is-open');
    secretsToggle.setAttribute('aria-expanded','true');
    secretsPanel.setAttribute('aria-hidden','false');
  };
  const closePanel = () => {
    secretsPanel.classList.remove('is-open','is-warm');
    secretsToggle.classList.remove('is-open');
    secretsToggle.setAttribute('aria-expanded','false');
    secretsPanel.setAttribute('aria-hidden','true');
    if(secretsMore) secretsMore.textContent = 'a little warmer';
  };
  secretsToggle.addEventListener('click', e=>{
    e.stopPropagation();
    if(!secretsPanel.classList.contains('is-open')){
      openPanel();
    } else if(!secretsPanel.classList.contains('is-warm')){
      secretsPanel.classList.add('is-warm');
      if(secretsMore) secretsMore.textContent = 'warmer hints shown';
    } else {
      closePanel();
    }
  });
  if(secretsMore){
    secretsMore.addEventListener('click', e=>{
      e.stopPropagation();
      secretsPanel.classList.add('is-warm');
      secretsMore.textContent = 'warmer hints shown';
    });
  }
  if(secretsClose){
    secretsClose.addEventListener('click', e=>{ e.stopPropagation(); closePanel(); });
  }
}

function setupHeartBreathe(){
  const targets = new Set();
  document.querySelectorAll('.intro-heart,.tiny-heart').forEach(el=>targets.add(el));
  slides.forEach(slide=>{
    const text = (slide.textContent || '').trim();
    if(text === '♥'){
      slide.querySelectorAll('p,span').forEach(el=>{
        if((el.textContent || '').trim() === '♥') targets.add(el);
      });
    }
  });
  targets.forEach(el=>{
    el.classList.add('heart-breathe-target');
    el.addEventListener('click', e=>{
      ...
      if(typeof flashToast === 'function'){
        flashToast(progressToast, 'take your time.', 2400);
        if(progressToast) progressToast.classList.add('breathe-toast');
        setTimeout(()=>progressToast && progressToast.classList.remove('breathe-toast'), 2500);
      }
    });
  });
}

createPetals(); setupCopyButton(); setupEasterEggs(); setupLittleSecrets(); setupHeartBreathe();

const videoIntro = document.getElementById('video-intro');
const introVideo = document.getElementById('intro-video');
const videoBypass = document.getElementById('video-bypass');
const videoFallback = document.getElementById('video-fallback');
const videoWish = document.getElementById('video-wish');
const videoMine = document.getElementById('video-mine');
...
function schedulePreludeCues(){
  if(!videoIntro || !introVideo) return;
  clearPreludeTimers();
  ...
  mineTimer = setTimeout(()=>{ videoIntro.classList.remove('show-wish'); videoIntro.classList.add('show-mine'); }, mineAt);
  washTimer = setTimeout(()=>{ videoIntro.classList.add('toward-heart'); }, washAt);
}
...
function beginVideoPrelude(){
  ...
  introVideo.addEventListener('loadedmetadata', schedulePreludeCues, { once:true });
