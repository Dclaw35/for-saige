...
document.addEventListener('keydown', e=>{ if(e.target.matches('input,textarea')) return; if(e.key==='ArrowRight'||e.key===' '||e.key==='Enter'){e.preventDefault();next(true);} if(e.key==='ArrowLeft'){e.preventDefault();prev();}});
let touchStartX = 0;
...
${letter}`); status.textContent = 'Copied. Now paste it into a text to me. The romance fax machine has delivered its sacred payload.'; } catch { status.textContent = 'Copy did not work automatically. Highlight the text and copy it the old fashioned way. Tragic, but survivable.'; } }); }
