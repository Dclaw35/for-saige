...
function showSlide(nextIndex, manual=false){ if(manual) userInteracted = true; clearTimeout(introTimer); index = Math.max(0, Math.min(total - 1, nextIndex)); const slide = slides[index]; slides.forEach((s,i)=>s.classList.toggle('active', i===index)); updateCounter(); progressBar.style.width = ((index+1)/total*100)+'%'; scrubber.value = String(index + 1); prevBtn.style.opacity = index===0 ? '.35' : '1'; nextBtn.textContent = index===total-1 ? 'Start Over' : 'Next'; if(index > 7) hint.style.opacity = '.28'; document.body.classList.toggle('quiet-mode', slide.classList.contains('quiet')); document.body.classList.toggle('dark-moment', slide.classList.contains('special-darken')); document.body.classList.toggle('tv-mode', slide.classList.contains('scene-tv')); document.body.classList.toggle('hush-mode', slide.classList.contains('scene-you') || slide.classList.contains('scene-cleanbeat')); document.body.classList.toggle('hush-lite', slide.classList.contains('scene-seen')); document.body.classList.toggle('scene-hush', slide.classList.contains('scene-hush')); if(progressEl && (slide.classList.contains('kind-chapter') || slide.classList.contains('kind-title'))){ progressEl.classList.remove('section-glow'); void progressEl.offsetWidth; progressEl.classList.add('section-glow'); setTimeout(()=>progressEl.classList.remove('section-glow'), 1300); } if(!userInteracted && !isPlaying && index < 3){ const introDur = Number(slide.dataset.duration || 2400); introTimer = setTimeout(()=>showSlide(index+1, false), introDur); } if(isPlaying) scheduleAutoplay(); }
